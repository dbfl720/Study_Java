...
// 부모클래스..??
//물품 리스트를 관리하고 CRUD 메소드가 이곳에 있어야 합니다.(필드와 메소드)
...
public class Market {
	...
	// 상품 추가
	public void insert(Goods goods) {  // 오버라이드 불러오기. Goods goods를 통해서.. 
		...
		// 판매 상태 유효성 확인
		if (isValidState(goods.getState()) == false) {  // ????? isValidaState 불러올때.. (String state)인데.. goods.getState() ? 
			System.out.println("판매상태가 유효하지 않습니다.");
			return;
		}
		
		boolean addResult = this.goodsList.add(goods); // 마지막에 상품 등록. 
		System.out.println(addResult ? "추가되었습니다." : "실패했습니다."); // 삼항연산자.
	}

	// 상품 출력
	public void read() { // input: x   output: x
...
// 			+ ", 가격:" + goods.getPrice() + ", 판매상태:" + goods.getState());
 //		}
	...
	private boolean isValidState(String state) { // private
		// 판매 상태 유효성 확인
